...
//        [--variants tools/msb_variants.json] [--resample 15m]   (--resample: ghép nến lên khung lớn → không có nến chi tiết)
...
import { TF_MS, resample } from "../js/timeframes.js";
...
const variantsPath = opt("--variants", new URL("./msb_variants.json", import.meta.url).pathname);
const resampleTo = opt("--resample", null);
...
if (resampleTo) K = resample(K, resampleTo);
...
const variants = JSON.parse(readFileSync(variantsPath));
const out = [];
...
const md = out.join("\n");
console.log(md);
if (process.env.GITHUB_STEP_SUMMARY && !process.env.NO_SUMMARY) appendFileSync(process.env.GITHUB_STEP_SUMMARY, md + "\n");
